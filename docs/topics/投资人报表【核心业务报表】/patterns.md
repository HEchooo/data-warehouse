...
- 正确口径应改为：按 `video_key` 全历史实际采集日中，最早满足 `raw_day_end_views > 0` 的 `dt` 记为新增日
- 这个“首次正播放采集日”是全历史属性；即使日常只重刷最近 30 天，上线新口径后也要补历史回填，不能指望旧分区自然修正
- 排查这类问题时，先逐个渠道核对：发布时间、多伦多时区下的首个采集时间、首个正播放采集日与发布日期的天数差，不要直接怀疑 ADS 聚合
